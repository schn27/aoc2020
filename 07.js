...
function calc() {
	...
	const part1 = Object.keys(bagsMap).filter(k => contains(bagsMap, k, "shiny gold")).length;

	const part2 = getNumberOfBags(bagsMap, "shiny gold");
	
	return part1 + " " + part2;
}

function contains(map, name, target) {
	if (map[name].findIndex(e => e.name == target) >= 0) {
		return true;
	} else {
		return map[name].reduce((a, b) => a | contains(map, b.name, target), false);
	}
}

function getNumberOfBags(map, name) {
	return map[name].reduce((a, b) => a + b.count * (1 + getNumberOfBags(map, b.name)), 0);
}

const input = `bright indigo bags contain 4 shiny turquoise bags, 3 wavy yellow bags.
