...
function calc() {
	const list = input.split("\n").map(s => {
		const tokens = s.match(/\d+|[a-z]+/g);
		return {p1: +tokens[0], p2: +tokens[1], char: tokens[2], pass: tokens[3].split("")};
	});

	const part1 = list.filter(e => {
		const count = e.pass.filter(c => c == e.char).length;
		return (count >= e.p1) && (count <= e.p2);
	}).length;

	const part2 = list.filter(e => (e.pass[e.p1 - 1] == e.char) ^ (e.pass[e.p2 - 1] == e.char)).length;

	return part1 + " " + part2;
}

const input = `15-16 m: mhmjmzrmmlmmmmmm
5-6 d: dcdddhzld
3-4 s: vqssdcbl
3-6 b: bbhbbbqbbb
10-11 q: cqqntqhqwwh
